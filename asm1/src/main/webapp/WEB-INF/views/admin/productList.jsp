							...
							<div class="row">
								...
								<div class="col-sm-4">
									<button type="button" class="btn btn-info add-new">
										<a href="productControl"><i class="fa fa-plus"></i> Add New</a>
									</button>
								</div>
							</div>
						</div>
							...
							<table class="table table-bordered table-hover">
								<thead>
									<tr>
										...
										<th style="width: 120px;">CreateDate</th>
								...
								<tbody>
									<c:forEach var="item" items="${page.content}">
										<tr>
											...
											<td>${item.id}</td>
											<td>${item.name}</td>
											<td><img src="/forUser/imgages/${item.image }" alt=""
									width=50% /></td>
											<td>${item.price}</td>
											<td>${item.createDate}</td>
											<td>${item.available}</td>
											<td>${item.category.name}</td>
											<td><a href="product/edit/${item.id}" class="edit" title="Edit" data-toggle="tooltip"><i
													class="material-icons">&#xE254;</i></a> 
													<a href="product/delete/${item.id}" class="delete"
							...
							<div class="col border-0">
								<div class=" col btn-group" role="group">
									<button type="button" class="btn border">
										<a href="/admin/product?p=0">First</a>
									</button>
									<button type="button" class="btn border">
										<a href="/admin/product?p=${page.number-1}">Previous</a>
									</button>
									<button type="button" class="btn border">
										<a href="/admin/product?p=${page.number+1}">Next</a>
									</button>
									<button type="button" class="btn border">
										<a href="/admin/product?p=${page.totalPages-1}">Last</a>
									</button>
								</div>
							</div>
							<div class="col">
								<small class="d-flex justify-content-end text-muted mt-2">Page: ${page.number}  of ${page.totalPages}</small>
							</div>
						</div>
